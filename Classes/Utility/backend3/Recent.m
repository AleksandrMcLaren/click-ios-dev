...

{
	[self fetchMembers:groupId completion:^(NSMutableArray *userIds)
	{
		...
		for (NSString *userId in createIds)
		{
			...
			[self createItem:userId groupId:groupId initials:[CKUser initials] picture:[CKUser picture] description:description
...

{
	[self fetchMembers:groupId completion:^(NSMutableArray *userIds)
	{
		...
		for (NSString *userId in createIds)
		{
			[self createItem:userId groupId:groupId initials:[CKUser initials] picture:picture description:description
...

{
	...
	if ([recent[FRECENT_USERID] isEqualToString:[CKUser currentId]] == NO) counter++;
...
+ (void)updateMembers:(CKDialogModel *)recent Members:(NSArray *)members
