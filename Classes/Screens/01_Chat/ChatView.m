...
//	NSString *groupId;
	...
	NSString *type;// need use class id _chat

	CKGroupChatModel *group;
...
- (id)initWithChat:(CKChatModel *)chat
{
    ...
    if ([chat isKindOfClass:[CKDialogChatModel class]]) {
        CKDialogChatModel* dialog = (CKDialogChatModel*) chat;
//        members = @[dialog.userId];
        type = CHAT_PRIVATE;

    }
    if ([chat isKindOfClass:[CKGroupChatModel class]]) {
        CKGroupChatModel* groupChat = (CKGroupChatModel*) chat;
//        members = groupChat.userIds;
        type = CHAT_GROUP;
...
//    [self performSelector:@selector(refreshCollectionView1)  withObject:self afterDelay:5];
	...
	UIMenuItem *menuItemCopy = [[UIMenuItem alloc] initWithTitle:@"Копировать" action:@selector(actionCopy:)];
	UIMenuItem *menuItemDelete = [[UIMenuItem alloc] initWithTitle:@"Удалить" action:@selector(actionDelete:)];
	UIMenuItem *menuItemSave = [[UIMenuItem alloc] initWithTitle:@"Сохранить" action:@selector(actionSave:)];
...
- (void)loadMessages
{
    ...
    [_chat.messagesDidChanged subscribeNext:^(NSArray *msgs) {
        dbmessages = msgs;
        [self refreshCollectionView1];
    }];
 
    dbmessages = _chat.messages;
    [self refreshCollectionView1];
...
//	CKUser *dbuser = [[DBUser objectsWithPredicate:predicate] firstObject];
//
...
- (void)messageSend:(NSString *)text Video:(NSURL *)video Picture:(UIImage *)picture Audio:(NSString *)audio{
	...
	{
        [_chat send:text Video:video Picture:picture Audio:audio];
    }
...
{
	Message* message = [self dbmessage:indexPath];
...

{
	...
	NSString *messageId = dbmessage.id ;
...
{
	Message *dbmessage = [self dbmessage:indexPath];
	NSString *senderId = dbmessage.userid;
...

{
	if ([self incoming:indexPath])
	{
		...
		if (indexPath.item > 0)
		{
			Message *dbprevious = [self dbmessage:[NSIndexPath indexPathForItem:indexPath.item-1 inSection:indexPath.section]];
			if ([dbprevious.userid isEqualToString:dbmessage.userid])
			{
				return nil;
			}
		}
...

{
	...
	{
		Message *dbmessage = [self dbmessage:indexPath];
		return [[NSAttributedString alloc] initWithString:dbmessage.statusName];
	}
	else return nil;
}
...
- (Message *)dbmessage:(NSIndexPath *)indexPath
...
{
	Message *dbmessage = [self dbmessage:indexPath];
	return jsqmessages[dbmessage.id];
}
