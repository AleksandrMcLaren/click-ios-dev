...
//	
//	for (DBUser *dbuser in [[DBUser allObjects] sortedResultsUsingProperty:CKUser_FULLNAME ascending:YES])
//	{
...
- (void)delGroupMember:(CKUser *)dbuser
...

{
	...
	[group[FGROUP_MEMBERS] removeObject:[CKUser currentId]];
	[group saveInBackground:^(NSError *error)
	{
		if (error == nil)
		{
...

{
	if (indexPath.section == 1)
	{
		...
		{
			CKUser *dbuser = dbusers[indexPath.row];
			return ([dbuser.objectId isEqualToString:[CKUser currentId]] == NO);
		}
	}
	return NO;
}
...
{
//	return [dbgroup.userId isEqualToString:[CKUser currentId]];
    return YES;
}
